...
--     Banded Report (Ratios)
--     Program Name, VA Supported Students, Non-Supported Students, Total Enrollment FTE, VA STudent % (FTE), Data of Calculation
--     Banded Report (Roster)
--     Program Name, First Initial, MI, Last Name, SSN (last 4 digits), VA of Non VA Student, FT or PT
--     CSV report
--     Program Name, Roster, VA or Non VA Student, FT or PT
--
-- Revision History
--
...
--Report Query: includes everything that is needed to create the report roster and the 85 percent Enrollment Ratios worksheet.
SELECT EnrollmentCohort.TERM_CODE,
       EnrollmentCohort.PIDM_KEY,
       EnrollmentCohort.MTSU_ID,
       EnrollmentCohort.FirstInit,
       EnrollmentCohort.MiddleInit,
       EnrollmentCohort.LastName,
       EnrollmentCohort.SSN,
       EnrollmentCohort.FirstInit || ' ' || CASE WHEN EnrollmentCohort.MiddleInit is not null then EnrollmentCohort.MiddleInit || ' ' END || LastName || ' - ' || EnrollmentCohort.SSN as Roster,
       EnrollmentCohort.Level_Code,
       ...
       --Set up student type of VA Student or Non VA Student
       --CCS JEP 5/28/2019 - Moved VETC_CODEs of N, X, and I to non-supported due to new template and definitions.
       --CCD CASE WHEN VeteranInfo.VETC_CODE IN ('A','B','C','D','F','G','I','T','N','E','P','Q','U') THEN '01' ELSE '02' END as SortBy,
       CASE WHEN VeteranInfo.VETC_CODE IN ('A','B','C','D','F','G','T','E','P','Q','U') THEN '01'
            ELSE '02' --Includes VETC_CODE IN ('I','N','X')
       END as SortBy,
       --CCD CASE WHEN VeteranInfo.VETC_CODE IN ('A','B','C','D','F','G','I','T','N','E','P','Q','U') THEN 'VA Students' ELSE 'Non VA Students' END as Student_Type,
       CASE WHEN VeteranInfo.VETC_CODE IN ('A','B','C','D','F','G','T','E','P','Q','U') THEN 'VA Students'
            ELSE 'Non VA Students' --Includes VETC_CODE IN ('I','N','X')
       END as Student_Type,
       --Determine if a student is full-time or part-time (12 hrs for UG or 9 hrs for GR) to be used when calculating the FTE per their definition.
       CASE WHEN (EnrollmentCohort.Level_Code = 'UG' AND EnrollmentCohort.CREDIT_HRS >= 12) OR (EnrollmentCohort.Level_Code = 'GR' AND EnrollmentCohort.CREDIT_HRS >= 9) THEN 'FT' ELSE 'PT' END as FullPartTime,
       --Add 1s and 0s for veterans and non veterans to sum for grand totals.
       --CCD CASE WHEN VeteranInfo.VETC_CODE IN ('A','B','C','D','F','G','I','T','N','E','P','Q','U') THEN 1 ELSE 0 END as VAStudentCnt,
       CASE WHEN VeteranInfo.VETC_CODE IN ('A','B','C','D','F','G','T','E','P','Q','U') THEN 1 ELSE 0 END as VAStudentCnt,
       --CCD CASE WHEN VeteranInfo.VETC_CODE is null THEN 1 ELSE 0 END as NonVAStudentCnt,
       CASE WHEN VeteranInfo.VETC_CODE is null OR VeteranInfo.VETC_CODE IN ('I','N','X') THEN 1 ELSE 0 END as NonVAStudentCnt,
       --Add 1s and 0s for veterans and non veterans full-time and part-time to sum for grand totals.
       ...
       CASE WHEN VeteranInfo.VETC_CODE IN ('A','B','C','D','F','G','T','E','P','Q','U') THEN
            CASE WHEN EnrollmentCohort.Level_Code = 'UG' THEN (CASE WHEN EnrollmentCohort.CREDIT_HRS >= 12 THEN 1 ELSE 0 END)
                 WHEN EnrollmentCohort.Level_Code = 'GR' THEN (CASE WHEN EnrollmentCohort.CREDIT_HRS >= 9 THEN 1 ELSE 0 END)
            END
            ELSE 0
       END as VAStudentsFT,
       --CCD CASE WHEN VeteranInfo.VETC_CODE IN ('A','B','C','D','F','G','I','T','N','E','P','Q','U') THEN
...
-- EnrollmentCohort
  (SELECT DISTINCT
         ...
         CASE
              --WHEN SGBSTDN_MAJR_CODE_2 IS NULL THEN SGBSTDN_MAJR_CODE_CONC_121
